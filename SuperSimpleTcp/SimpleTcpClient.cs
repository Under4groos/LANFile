...
public class SimpleTcpClient : IDisposable
{
    ...
    public IPEndPoint LocalEndpoint
    {
        get
        {
            if (_client != null && IsConnected) return (IPEndPoint)_client.Client.LocalEndPoint;
    ...
    /// </summary>
    public SimpleTcpStatistics Statistics { get; } = new();
    ...
    private SimpleTcpClientSettings _settings = new();
    private SimpleTcpClientEvents _events = new();
    ...
    public SimpleTcpClient(IPEndPoint serverIpEndPoint)
    {
        ...
        if (serverIpEndPoint.Port < 0) throw new ArgumentException("Port must be zero or greater.");
    ...
    /// <param name="pfxCertFilename">The filename of the PFX certificate file.</param>
    /// <param name="pfxPassword">The password to the PFX certificate file.</param>
    public SimpleTcpClient(IPEndPoint serverIpEndPoint, bool ssl, string pfxCertFilename, string pfxPassword) : this(
        serverIpEndPoint)
    {
        _ssl = ssl;
        _pfxCertFilename = pfxCertFilename;
    ...
    public void Connect()
    {
        ...
        if (_ssl)
        {
            ...
            _sslStream.AuthenticateAsClient(_serverIp, _sslCertCollection, SslProtocols.Tls12,
    ...
    /// <summary>
    ///     Establish the connection to the server with retries up to either the timeout specified or the value in
    ...
    public void ConnectWithRetries(int? timeoutMs = null)
    {
        ...
        using (var connectTokenSource = new CancellationTokenSource())
        {
            ...
            var cancelTask = Task.Delay(_settings.ConnectTimeoutMs, _token);
            var connectTask = Task.Run(() =>
            {
                ...
                while (true)
                    try
                    {
                        var msg = $"{_header}attempting connection to {_serverIp}:{_serverPort}";
                        if (retryCount > 0) msg += $" ({retryCount} retries)";
                        Logger?.Invoke(msg);

                        _client.Dispose();
                        _client = _settings.LocalEndpoint == null
                            ? new TcpClient()
                            : new TcpClient(_settings.LocalEndpoint);
    ...
    private bool AcceptCertificate(object sender, X509Certificate certificate, X509Chain chain,
    ...
    private async Task<ArraySegment<byte>> DataReadAsync(CancellationToken token)
    {
        ...
        try
        {
            ...
            var tcpConnections = ipProperties.GetActiveTcpConnections()
                .Where(x => x.LocalEndPoint.Equals(_client.Client.LocalEndPoint) &&
    ...
    private void EnableKeepalives()
    {
        ...
        try
        {
            ...
            _client.Client.SetSocketOption(SocketOptionLevel.Tcp, SocketOptionName.TcpKeepAliveTime,
                _keepalive.TcpKeepAliveTime);
            _client.Client.SetSocketOption(SocketOptionLevel.Tcp, SocketOptionName.TcpKeepAliveInterval,
                _keepalive.TcpKeepAliveInterval);

            // Windows 10 version 1703 or later

            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows)
                && Environment.OSVersion.Version >= new Version(10, 0, 15063))
                _client.Client.SetSocketOption(SocketOptionLevel.Tcp, SocketOptionName.TcpKeepAliveRetryCount,
                    _keepalive.TcpKeepAliveRetryCount);
