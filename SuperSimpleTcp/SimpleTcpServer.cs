...
public class SimpleTcpServer : IDisposable
{
    ...
    {
        var clients = new List<string>(_clients.Keys);
        return clients;
    }
        ...
        while (!_listenerToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                if (_ssl)
                {
                    ...
                    var tlsCts = CancellationTokenSource.CreateLinkedTokenSource(_listenerToken, _token);
                    tlsCts.CancelAfter(3000);

                    var success = await StartTls(client, tlsCts.Token).ConfigureAwait(false);
                    if (!success)
                    {
                        client.Dispose();
                        continue;
                ...
                var unawaited = Task.Run(() => DataReceiver(client), linkedCts.Token);
    ...
    private async Task DataReceiver(ClientMetadata client)
    {
        ...
        while (true)
            try
            {
                ...
                var action = () => _events.HandleDataReceived(this, new DataReceivedEventArgs(ipPort, data));
            ...
            {
                Logger?.Invoke(
                    $"{_header}data receiver exception [{ipPort}]:{Environment.NewLine}{e}{Environment.NewLine}");
                break;
            }
    ...
    private async Task IdleClientMonitor()
    {
        while (!_token.IsCancellationRequested)
        {
            ...
            try
            {
                var idleTimestamp = DateTime.Now.AddMilliseconds(-1 * _settings.IdleClientTimeoutMs);

                foreach (var curr in _clientsLastSeen)
    ...
    {
        if (_clientsLastSeen.ContainsKey(ipPort)) _clientsLastSeen.TryRemove(ipPort, out _);
    ...
    {
        if (!_clients.TryGetValue(ipPort, out var client)) return;
    ...
    private void EnableKeepalives()
    {
        ...
        try
        {
            ...
            _listener.Server.SetSocketOption(SocketOptionLevel.Tcp, SocketOptionName.TcpKeepAliveTime,
                _keepalive.TcpKeepAliveTime);
            _listener.Server.SetSocketOption(SocketOptionLevel.Tcp, SocketOptionName.TcpKeepAliveInterval,
                _keepalive.TcpKeepAliveInterval);
            _listener.Server.SetSocketOption(SocketOptionLevel.Tcp, SocketOptionName.TcpKeepAliveRetryCount,
    ...
    private void EnableKeepalives(TcpClient client)
    {
        try
        {
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows)
                ...
                client.Client.SetSocketOption(SocketOptionLevel.Tcp, SocketOptionName.TcpKeepAliveRetryCount,
                    _keepalive.TcpKeepAliveRetryCount);
